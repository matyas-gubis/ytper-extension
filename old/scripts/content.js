...
function handleReceivedWord(receivedWord) {
  if (keymapIndex < 3 && receivedWord.length === 9) {
    ...
    originalKeymap.split("").forEach((letter, i) => {
      document.getElementById(`Y-keycap-${letter}`).textContent =
        mappedKeymap.charAt(i);
    });
  } else if (
    keymapIndex >= 3 &&
    shouldSolve &&
...
function createKeyboard() {
  ...
  return keyboard = document.getElementById("Y-keyboard");
}
...
function getDelay() {
  return (
    Math.floor(Math.random() * MAX_SEND_DELAY - MIN_SEND_DELAY) + MIN_SEND_DELAY
  );
}
